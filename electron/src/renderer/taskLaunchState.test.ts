...
describe('Chinese IA surface scoping', () => {
  it('keeps strategy and backtest surfaces distinct', () => {
    const signalsDashboard = {
      ...
      connector_health: [{ connector_id: 'signals-web', status: 'degraded', summary: 'lagging' }],
      diagnostics: [{ diagnostic_id: 'diag-1', label: 'signals-web', detail: 'lagging', status: 'degraded' }],
      deep_links: [{ link_id: 'link-1', label: 'Open Signals', url: 'https://example.com' }],
      operator_actions: [],
      recent_runs: [],
    }
